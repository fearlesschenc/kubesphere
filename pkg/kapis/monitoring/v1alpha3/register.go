...
func AddToContainer(c *restful.Container, k8sClient kubernetes.Interface, monitoringClient monitoring.Interface, factory informers.InformerFactory, opClient openpitrix.Client) error {
	...
	ws.Route(ws.GET("/kubesphere").
		...
		Writes(Metrics{}).
		Returns(http.StatusOK, respOK, Metrics{})).
		Produces(restful.MIME_JSON)

	ws.Route(ws.GET("/cluster").
		To(h.getClusterMetrics).
		...
		Param(ws.QueryParameter("start", "Start time of queryMetrics. Use **start** and **end** to retrieve metric data over a time span. It is a string with Unix time format, eg. 1559347200. ").DataType("string").Required(false)).
		Param(ws.QueryParameter("end", "End time of queryMetrics. Use **start** and **end** to retrieve metric data over a time span. It is a string with Unix time format, eg. 1561939200. ").DataType("string").Required(false)).
	...
	ws.Route(ws.GET("/components/{component}").
		To(h.getComponentMetrics).
		Doc("Get component-level metric data of the specific system component.").
		Param(ws.PathParameter("component", "system component to monitoring. One of etcd, apiserver, scheduler.").DataType("string").Required(true)).
	...
	ws.Route(ws.GET("/namespaces/{namespace}/targets/metadata").
		...
		Writes(Metadata{}).
		Returns(http.StatusOK, respOK, Metadata{})).
		Produces(restful.MIME_JSON)

	ws.Route(ws.GET("/namespaces/{namespace}/targets/labelsets").
		To(h.getMetricLabelSet).
		...
		Param(ws.QueryParameter("start", "Start time of queryMetrics. It is a string with Unix time format, eg. 1559347200. ").DataType("string").Required(true)).
		Param(ws.QueryParameter("end", "End time of queryMetrics. It is a string with Unix time format, eg. 1561939200. ").DataType("string").Required(true)).
		Metadata(restfulspec.KeyOpenAPITags, []string{constants.CustomMetricsTag}).
		Writes(MetricLabelSet{}).
		Returns(http.StatusOK, respOK, MetricLabelSet{})).
		Produces(restful.MIME_JSON)

	ws.Route(ws.GET("/namespaces/{namespace}/targets/queryMetrics").
		To(h.adhocQuery).
		Doc("Make an ad-hoc queryMetrics in the specific namespace.").
		Param(ws.PathParameter("namespace", "The name of the namespace.").DataType("string").Required(true)).
